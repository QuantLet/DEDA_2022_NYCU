...
# "car"       "carData"   "mvtnorm"   "MASS"      "stats"     
# "graphics"  "grDevices" "utils"     "datasets"  "methods"   "base"  


# load data
bond <- read.csv("bond.csv", row.names = 1) 
f       = scale(bond)                           # standardize variables
...
# Plot 1: PCA
dev.new()
plot(ym, type = "n", xlab = "first PC", ylab = "second PC", main = "BOND data", 
...
title(main = "Ward Dendrogram for BOND", ylab = "Squared Euclidean Distance")
...
dev.new()
plot(ym, type = "n", xlab = "first PC", ylab = "second PC", main = "BOND data, cut height 60", 
     ylim = c(-5, 4), xlim = c(-5, 4.5))
text(ym[, 1], ym[, 2], rownames(f), cex = 1.2)
